...
object TimeUsage {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    val stringColumns = StructField(columnNames.head, StringType, false)
    val doubleColumns = columnNames.tail.map(StructField(_, DoubleType, false))
    StructType(stringColumns :: doubleColumns)
  }
  ...
  def timeUsageSummary(
    ...
    val ageProjection: Column = when($"teage">=15 && $"teage"<=22, "young")
      .when($"teage">=23 && $"teage"<=55, "active").otherwise("elder")

    // Create columns that sum columns of the initial dataset
    // Hint: you want to create a complex column expression that sums other columns
    //       by using the `+` operator between them
    // Hint: don’t forget to convert the value to hours
    val primaryNeedsProjection: Column = (primaryNeedsColumns.reduceLeft(_ + _).cast(DoubleType) / 60).as("primaryNeeds")
    val workProjection: Column = (workColumns.reduceLeft(_ + _).cast(DoubleType)/60).as("work")
    val otherProjection: Column = (otherColumns.reduceLeft(_ + _).cast(DoubleType)/60).as("other")
    df
  ...
  def timeUsageGroupedSqlQuery(viewName: String): String =
    "select working, sex, age, round(avg(primaryNeeds), 1) as primaryNeeds, " +
      "round(avg(work), 1) as work, " +
      "round(avg(other), 1) as other from summed group by working, sex, age order by working, sex, age"
